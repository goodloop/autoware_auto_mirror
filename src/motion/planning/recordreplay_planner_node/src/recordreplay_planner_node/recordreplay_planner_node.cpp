  ...
  const geometry_msgs::msg::TransformStamped & transform)
{
  ...
  geometry_msgs::msg::PoseStamped source_pose, target_pose;
  source_pose.pose.position = toPoint(source_msg.centroid);
  source_pose.pose.orientation = toQuaternion(source_msg.orientation);
  tf2::doTransform(source_pose, target_pose, transform);
  target_msg.centroid = toPoint32(target_pose.pose.position);
  target_msg.orientation = toQuaternion32(target_pose.pose.orientation);
