...
TEST_F(MPCTest, initialize_and_calculate) {
  ...
  ASSERT_TRUE(mpc.calculateMPC(neutral_steer, default_velocity, pose_zero, ctrl_cmd, pred_traj));
...
TEST_F(MPCTest, osqp_calculate) {
  ...
  EXPECT_FALSE(mpc.calculateMPC(neutral_steer, default_velocity, pose_zero, ctrl_cmd, pred_traj));
  EXPECT_EQ(ctrl_cmd.steering_tire_angle, 0.0f);
  EXPECT_EQ(ctrl_cmd.steering_tire_rotation_rate, 0.0f);
}
...
TEST_F(MPCTest, failure_cases) {
  ...
  AckermannLateralCommand ctrl_cmd;
  Trajectory pred_traj;
  EXPECT_FALSE(mpc.calculateMPC(neutral_steer, default_velocity, pose_far, ctrl_cmd, pred_traj));

  // Calculate MPC with a fast velocity to make the prediction go further than the reference path
  EXPECT_FALSE(
    mpc.calculateMPC(neutral_steer, default_velocity + 10.0, pose_far, ctrl_cmd, pred_traj));
  ...
  EXPECT_TRUE(mpc.calculateMPC(neutral_steer, default_velocity, pose_zero, ctrl_cmd, pred_traj));
}
}  // namespace
