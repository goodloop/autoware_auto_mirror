...
: Node("longitudinal_controller", node_options), m_tf_listener(m_tf_buffer, this, false)
{
  ...
  // parameters timer
  m_control_rate = declare_parameter<double>("control_rate");

  const float64_t cog_to_rear_axle = declare_parameter<double>("vehicle.cg_to_rear_m");
  const float64_t cog_to_front_axle = declare_parameter<double>("vehicle.cg_to_front_m");
  m_wheel_base = cog_to_rear_axle + cog_to_front_axle;

  // parameters for delay compensation
  m_delay_compensation_time = declare_parameter<double>("delay_compensation_time");  // [s]

  // parameters to enable functions
  m_enable_smooth_stop = declare_parameter<bool>("enable_smooth_stop");
  m_enable_overshoot_emergency = declare_parameter<bool>("enable_overshoot_emergency");
  m_enable_slope_compensation = declare_parameter<bool>("enable_slope_compensation");

  // parameters for state transition
  {
    auto & p = m_state_transition_params;
    // drive
    p.drive_state_stop_dist = declare_parameter<double>("drive_state_stop_dist");  // [m]
    p.drive_state_offset_stop_dist = declare_parameter<double>(
      "drive_state_offset_stop_dist");  // [m]
    // stopping
    p.stopping_state_stop_dist =
      declare_parameter<double>("stopping_state_stop_dist");  // [m]
    // stop
    p.stopped_state_entry_vel =
      declare_parameter<double>("stopped_state_entry_vel");  // [m/s]
    p.stopped_state_entry_acc =
      declare_parameter<double>("stopped_state_entry_acc");  // [m/s²]
    // emergency
    p.emergency_state_overshoot_stop_dist = declare_parameter<double>(
      "emergency_state_overshoot_stop_dist");  // [m]
    p.emergency_state_traj_trans_dev = declare_parameter<double>(
      "emergency_state_traj_trans_dev");  // [m]
    p.emergency_state_traj_rot_dev = declare_parameter<double>(
  ...
  {
    // initialize PID gain
    const float64_t kp{declare_parameter<double>("kp")};
    const float64_t ki{declare_parameter<double>("ki")};
    const float64_t kd{declare_parameter<double>("kd")};
    m_pid_vel.setGains(kp, ki, kd);

    // initialize PID limits
    const float64_t max_pid{declare_parameter<double>("max_out")};     // [m/s^2]
    const float64_t min_pid{declare_parameter<double>("min_out")};     // [m/s^2]
    const float64_t max_p{declare_parameter<double>("max_p_effort")};  // [m/s^2]
    const float64_t min_p{declare_parameter<double>("min_p_effort")};  // [m/s^2]
    const float64_t max_i{declare_parameter<double>("max_i_effort")};  // [m/s^2]
    const float64_t min_i{declare_parameter<double>("min_i_effort")};  // [m/s^2]
    const float64_t max_d{declare_parameter<double>("max_d_effort")};  // [m/s^2]
    const float64_t min_d{declare_parameter<double>("min_d_effort")};  // [m/s^2]
    m_pid_vel.setLimits(max_pid, min_pid, max_p, min_p, max_i, min_i, max_d, min_d);

    // set lowpass filter for vel error and pitch
    const float64_t lpf_vel_error_gain{declare_parameter<double>("lpf_vel_error_gain")};
    m_lpf_vel_error =
      std::make_shared<trajectory_follower::LowpassFilter1d>(0.0, lpf_vel_error_gain);

    m_current_vel_threshold_pid_integrate = declare_parameter<double>(
      "current_vel_threshold_pid_integration");  // [m/s]
  }

  // parameters for smooth stop state
  {
    const float64_t max_strong_acc{declare_parameter<double>(
        "smooth_stop_max_strong_acc")};         // [m/s^2]
    const float64_t min_strong_acc{declare_parameter<double>(
        "smooth_stop_min_strong_acc")};         // [m/s^2]
    const float64_t weak_acc{declare_parameter<double>(
        "smooth_stop_weak_acc")};               // [m/s^2]
    const float64_t weak_stop_acc{declare_parameter<double>(
        "smooth_stop_weak_stop_acc")};          // [m/s^2]
    const float64_t strong_stop_acc{declare_parameter<double>(
        "smooth_stop_strong_stop_acc")};        // [m/s^2]

    const float64_t max_fast_vel{declare_parameter<double>(
        "smooth_stop_max_fast_vel")};            // [m/s]
    const float64_t min_running_vel{declare_parameter<double>(
        "smooth_stop_min_running_vel")};        // [m/s]
    const float64_t min_running_acc{declare_parameter<double>(
        "smooth_stop_min_running_acc")};        // [m/s^2]
    const float64_t weak_stop_time{declare_parameter<double>(
        "smooth_stop_weak_stop_time")};          // [s]

    const float64_t weak_stop_dist{declare_parameter<double>(
        "smooth_stop_weak_stop_dist")};         // [m]
    const float64_t strong_stop_dist{declare_parameter<double>(
  ...
  // parameters for stop state
  {
    ...
    p.vel = declare_parameter<double>("stopped_vel");   // [m/s]
    p.acc = declare_parameter<double>("stopped_acc");  // [m/s^2]
    p.jerk = declare_parameter<double>("stopped_jerk");  // [m/s^3]
  }

  // parameters for emergency state
  {
    auto & p = m_emergency_state_params;
    p.vel = declare_parameter<double>("emergency_vel");     // [m/s]
    p.acc = declare_parameter<double>("emergency_acc");    // [m/s^2]
    p.jerk = declare_parameter<double>("emergency_jerk");  // [m/s^3]
  }

  // parameters for acceleration limit
  m_max_acc = declare_parameter<double>("max_acc");   // [m/s^2]
  m_min_acc = declare_parameter<double>("min_acc");  // [m/s^2]

  // parameters for jerk limit
  m_max_jerk = declare_parameter<double>("max_jerk");   // [m/s^3]
  m_min_jerk = declare_parameter<double>("min_jerk");  // [m/s^3]

  // parameters for slope compensation
  m_use_traj_for_pitch = declare_parameter<bool>("use_trajectory_for_pitch_calculation");
  const float64_t lpf_pitch_gain{declare_parameter<double>("lpf_pitch_gain")};
  m_lpf_pitch = std::make_shared<trajectory_follower::LowpassFilter1d>(0.0, lpf_pitch_gain);
  m_max_pitch_rad = declare_parameter<double>("max_pitch_rad");   // [rad]
  m_min_pitch_rad = declare_parameter<double>("min_pitch_rad");  // [rad]
