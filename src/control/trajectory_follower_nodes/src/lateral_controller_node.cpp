...
: Node("lateral_controller", node_options), m_tf_listener(m_tf_buffer, this, false)
{
  ...
  m_mpc.m_ctrl_period = declare_parameter<double>("ctrl_period");
  m_enable_path_smoothing = declare_parameter<bool>("enable_path_smoothing");
  m_enable_yaw_recalculation = declare_parameter<bool>("enable_yaw_recalculation");
  m_path_filter_moving_ave_num = declare_parameter<int>("path_filter_moving_ave_num");
  m_curvature_smoothing_num = declare_parameter<int>("curvature_smoothing_num");
  m_traj_resample_dist = declare_parameter<double>("traj_resample_dist");
  m_mpc.m_admissible_position_error =
    declare_parameter<double>("admissible_position_error");
  m_mpc.m_admissible_yaw_error_rad = declare_parameter<double>("admissible_yaw_error_rad");
  m_mpc.m_use_steer_prediction = declare_parameter<bool>("use_steer_prediction");
  m_mpc.m_param.steer_tau = declare_parameter<double>("vehicle_model_steer_tau");

  /* stop state parameters */
  m_stop_state_entry_ego_speed = declare_parameter<double>("stop_state_entry_ego_speed");
  m_stop_state_entry_target_speed =
    declare_parameter<double>("stop_state_entry_target_speed");
  m_stop_state_keep_stopping_dist =
    declare_parameter<double>("stop_state_keep_stopping_dist");

  /* mpc parameters */
  const float64_t steer_lim_deg = declare_parameter<double>("steer_lim_deg");
  const float64_t steer_rate_lim_degs = declare_parameter<double>("steer_rate_lim_dps");
  ...
  const float64_t cg_to_front_m = declare_parameter<double>("vehicle.cg_to_front_m");
  const float64_t cg_to_rear_m = declare_parameter<double>("vehicle.cg_to_rear_m");
  ...
  if (vehicle_model_type == "kinematics") {
    ...
    const float64_t mass_fl = declare_parameter<double>("vehicle.mass_fl");
    const float64_t mass_fr = declare_parameter<double>("vehicle.mass_fr");
    const float64_t mass_rl = declare_parameter<double>("vehicle.mass_rl");
    const float64_t mass_rr = declare_parameter<double>("vehicle.mass_rr");
    const float64_t cf = declare_parameter<double>("vehicle.cf");
    const float64_t cr = declare_parameter<double>("vehicle.cr");
  ...
  /* delay compensation */
  {
    const float64_t delay_tmp = declare_parameter<double>("input_delay");
  ...
  /* initialize lowpass filter */
  {
    const float64_t steering_lpf_cutoff_hz =
      declare_parameter<double>("steering_lpf_cutoff_hz");
    const float64_t error_deriv_lpf_cutoff_hz =
      declare_parameter<double>("error_deriv_lpf_cutoff_hz");
  ...
  m_pub_predicted_traj =
    ...
    "output/lateral/predicted_trajectory", 1);
  m_pub_diagnostic =
...
void LateralController::onState(
...
{
  m_mpc.m_param.prediction_horizon = declare_parameter<int>("mpc_prediction_horizon");
  m_mpc.m_param.prediction_dt = declare_parameter<double>("mpc_prediction_dt");
  m_mpc.m_param.weight_lat_error = declare_parameter<double>("mpc_weight_lat_error");
  m_mpc.m_param.weight_heading_error =
    declare_parameter<double>("mpc_weight_heading_error");
  m_mpc.m_param.weight_heading_error_squared_vel = declare_parameter<double>(
    "mpc_weight_heading_error_squared_vel");
  m_mpc.m_param.weight_steering_input =
    declare_parameter<double>("mpc_weight_steering_input");
  m_mpc.m_param.weight_steering_input_squared_vel = declare_parameter<double>(
    "mpc_weight_steering_input_squared_vel");
  m_mpc.m_param.weight_lat_jerk = declare_parameter<double>("mpc_weight_lat_jerk");
  m_mpc.m_param.weight_steer_rate = declare_parameter<double>("mpc_weight_steer_rate");
  m_mpc.m_param.weight_steer_acc = declare_parameter<double>("mpc_weight_steer_acc");
  m_mpc.m_param.low_curvature_weight_lat_error = declare_parameter<double>(
    "mpc_low_curvature_weight_lat_error");
  m_mpc.m_param.low_curvature_weight_heading_error = declare_parameter<double>(
    "mpc_low_curvature_weight_heading_error");
  m_mpc.m_param.low_curvature_weight_heading_error_squared_vel = declare_parameter<double>(
    "mpc_low_curvature_weight_heading_error_squared_vel");
  m_mpc.m_param.low_curvature_weight_steering_input = declare_parameter<double>(
    "mpc_low_curvature_weight_steering_input");
  m_mpc.m_param.low_curvature_weight_steering_input_squared_vel = declare_parameter<double>(
    "mpc_low_curvature_weight_steering_input_squared_vel");
  m_mpc.m_param.low_curvature_weight_lat_jerk = declare_parameter<double>(
    "mpc_low_curvature_weight_lat_jerk");
  m_mpc.m_param.low_curvature_weight_steer_rate = declare_parameter<double>(
    "mpc_low_curvature_weight_steer_rate");
  m_mpc.m_param.low_curvature_weight_steer_acc = declare_parameter<double>(
    "mpc_low_curvature_weight_steer_acc");
  m_mpc.m_param.low_curvature_thresh_curvature = declare_parameter<double>(
    "mpc_low_curvature_thresh_curvature");
  m_mpc.m_param.weight_terminal_lat_error =
    declare_parameter<double>("mpc_weight_terminal_lat_error");
  m_mpc.m_param.weight_terminal_heading_error = declare_parameter<double>(
    "mpc_weight_terminal_heading_error");
  m_mpc.m_param.zero_ff_steer_deg = declare_parameter<double>("mpc_zero_ff_steer_deg");
  m_mpc.m_param.acceleration_limit = declare_parameter<double>("mpc_acceleration_limit");
  m_mpc.m_param.velocity_time_constant =
    declare_parameter<double>("mpc_velocity_time_constant");
}
