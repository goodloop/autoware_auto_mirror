...
: ControllerBaseNode{name, ns}
{
  ...
  const BehaviorConfig behavior{
    ...
    std::chrono::milliseconds(declare_parameter<int>("controller.behavior.time_step_ms")),
    ref_type
  };
  ...
  const auto sample_tolerance_ms =
    std::chrono::milliseconds(declare_parameter<int>("controller.sample_tolerance_ms"));

  const auto control_lookahead_ms =
    std::chrono::milliseconds(declare_parameter<int>("controller.control_lookahead_ms"));
  ...
  const auto debug_cycle_duration_param = declare_parameter<int>("debug_trajectory_publish_period_ms");
  const auto cycle_duration =
    std::chrono::milliseconds{debug_cycle_duration_param};
