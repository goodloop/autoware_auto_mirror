...
: TrajectoryPlannerNodeBase{"lane_planner", "plan_lane_trajectory", node_options}
{
  ...
  const TrajectorySmootherConfig config{
    static_cast<float32_t>(
      declare_parameter<double>("gaussian_smoother.standard_deviation")),
    static_cast<uint32_t>(declare_parameter<int>("gaussian_smoother.kernel_size"))
  };
  const lane_planner::LanePlannerConfig planner_config{
    static_cast<float32_t>(
      declare_parameter<double>("lane_planner.trajectory_resolution"))
  };
