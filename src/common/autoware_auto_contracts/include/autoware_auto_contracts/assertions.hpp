...
#ifndef AUTOWARE_AUTO_CONTRACTS__ASSERTIONS_HPP_
#define AUTOWARE_AUTO_CONTRACTS__ASSERTIONS_HPP_
...
contracts_lite::ReturnStatus vector_below_capacity(const std::vector<T> & v)
{
  ...
  auto comment = CONTRACT_COMMENT("", "The vector's size (" + std::to_string(
        v.size()) + ") must not exceed its capacity (" + std::to_string(v.capacity()) + ")");
...
#endif  // AUTOWARE_AUTO_CONTRACTS__ASSERTIONS_HPP_
