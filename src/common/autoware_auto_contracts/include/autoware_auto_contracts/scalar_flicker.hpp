...
public:
   ...
   */
  static bool is_valid_for_array(const std::array<T, S> & arr, const PeakIndex & peak);
...
: p1_idx(p1_idx), p2_idx(p2_idx), p3_idx(p3_idx)
{
  DEFAULT_ENFORCE([&]() {
      ...
      "Input indices must be ordered with p1_idx (" +
      std::to_string(p1_idx) + ") < p2_idx (" + std::to_string(p2_idx) + ") < p3_idx (" +
      std::to_string(p3_idx) + ")");
      return contracts_lite::ReturnStatus(std::move(comment),
      PeakIndex<T, S>::indices_are_valid(p1_idx, p2_idx,
      p3_idx));
    } ());
}
...
{
  return "[" + std::to_string(peak.p1_idx) + ", " + std::to_string(peak.p2_idx) + ", " +
         std::to_string(peak.p3_idx) + "]";
}

//------------------------------------------------------------------------------

template<typename T, size_t S>
bool PeakIndex<T, S>::indices_are_valid(size_t p1_idx, size_t p2_idx, size_t p3_idx)
{
...
template<typename T, size_t S>
bool PeakIndex<T, S>::is_valid_for_array(const std::array<T, S> & arr, const PeakIndex<T, S> & peak)
{
...
{
  static_assert(S > 2, "An array requires at least three elements to have a peak.");
 ...
 */
template<typename T, size_t S>
PeakIndex<T, S> greatest_magnitude_peak(const std::array<T, S> & arr, size_t begin, size_t end)
{
  static_assert(S > 2, "An array requires at least three elements to have a peak.");
...
PeakIndex<T, S> greatest_magnitude_peak(const std::array<T, S> & arr)
{
  ...
  auto max_mag = static_cast<T>(0);
  auto max_peak = PeakIndex<T, S>(p1_idx_initial, p2_idx_initial, p3_idx_initial);
...
: limit_(std::move(limit)), name_("[" + name + "]: ")
{
  DEFAULT_ENFORCE([&]() {
      ...
      ") must be strictly positive.", (limit > static_cast<T>(0)));
    } ());
...
contracts_lite::ReturnStatus ScalarFlicker<T, S>::no_flicker(T val)
{
  ...
  auto comment = CONTRACT_COMMENT("", (name_ + "Magnitude (" + gcc_7x_to_string_fix(
        mag) + ") of peak at " + PeakIndex<T, S>::string(
        peak) + " must be less than the limit (" + gcc_7x_to_string_fix(limit_) + ")"));
