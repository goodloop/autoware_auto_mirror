...
: Node("Lanelet2MapProvider", options)
{
  ...
  const float64_t origin_offset_lat = declare_parameter<double>("origin_offset_lat", 0.0);
  const float64_t origin_offset_lon = declare_parameter<double>("origin_offset_lon", 0.0);
  if (has_parameter("latitude") && has_parameter("longitude") && has_parameter("elevation")) {
    const float64_t origin_lat = declare_parameter<double>("latitude");
    const float64_t origin_lon = declare_parameter<double>("longitude");
    const float64_t origin_alt = declare_parameter<double>("elevation");
