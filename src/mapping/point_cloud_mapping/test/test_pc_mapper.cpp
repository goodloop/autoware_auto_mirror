...
TEST_F(PCMapperTest, core) {
  ...
  {
    auto localizer_ptr = std::make_unique<MockLocalizer>(m_tf1, m_tf2);
    auto pc_map_ptr = std::make_unique<PlainPointCloudMap>(m_pc1.width + m_pc2.width, map_frame);
    PointCloudMapper<MockLocalizer, CapacityTrigger, MockPrefixGenerator>
    mapper{fn_prefix, std::move(localizer_ptr),
      std::move(pc_map_ptr), map_frame};
    mapper.observe(m_pc1, geometry_msgs::msg::TransformStamped{});
    mapper.observe(m_pc2, geometry_msgs::msg::TransformStamped{});
  }
  PCLCloud final_map;
