// Copyright 2017-2021 Arm Ltd., TierIV, Autoware Foundation, The Apollo Authors
//
...
#ifndef APOLLO_LIDAR_SEGMENTATION__CLUSTER2D_HPP_
#define APOLLO_LIDAR_SEGMENTATION__CLUSTER2D_HPP_
...
enum class MetaType { META_UNKNOWN, META_SMALLMOT, META_NONMOT, META_PEDESTRIAN, MAX_META_TYPE };
...
struct Obstacle
{
  ...
  Obstacle()
  : score(0.0), height(-5.0), meta_type(MetaType::META_UNKNOWN)
  {
...
///        cells.
class APOLLO_LIDAR_SEGMENTATION_LOCAL Cluster2D
{
public:
  /// \brief Constructor
  ...
  void cluster(
    const float32_t * inferred_data,
    const pcl::PointCloud<pcl::PointXYZI>::ConstPtr & pc_ptr,
    const pcl::PointIndices & valid_indices,
    float32_t objectness_thresh, bool8_t use_all_grids_for_clustering);
...
#endif  // APOLLO_LIDAR_SEGMENTATION__CLUSTER2D_HPP_
