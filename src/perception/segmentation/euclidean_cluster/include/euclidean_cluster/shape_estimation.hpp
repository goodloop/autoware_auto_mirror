...
{
// All the different bounding box computation methods are defined here as structs.
// This is because the upstream module is expected to use `variant` to choose between
// `DetectedObjects` and `BoundingBoxArray` types.
// Having these structs makes it easy to use `visit` to call the appropriate function based on
// the underlying type held by the `variant`.

/// \brief Compute lfit bounding boxes for the given cluster
...
/// \brief Compute lfit bounding boxes with z for the given cluster
...
/// \brief Compute eigen bounding boxes for the given cluster
...
/// \brief Compute eigen bounding boxes with z for the given cluster
