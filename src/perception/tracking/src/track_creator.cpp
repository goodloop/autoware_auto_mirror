...
TracksAndLeftovers LidarClusterIfVisionPolicy::create()
{
  ...
  std::cerr << "Unassigned lidar " << m_lidar_clusters.objects.size() << " vision " << vision_msg.rois.size() << " now assigned " << m_lidar_clusters.objects.size() - result.unassigned_track_indices.size() << std::endl;
