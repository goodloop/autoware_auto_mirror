...
PointCloudDensification::PointCloudDensification(
  std::string base_frame_id, const unsigned int pointcloud_cache_size, rclcpp::Clock::SharedPtr clock)
: base_frame_id_(std::move(base_frame_id)), pointcloud_cache_size_(pointcloud_cache_size), tf_buffer_(clock)
{
}
  ...
  const sensor_msgs::msg::PointCloud2 & input_pointcloud_msg)
{
  ...
  for (; pc_msg_iter != pointcloud_cache_.end(); pc_msg_iter++, matrix_iter++) {
    ...
    double diff_timestamp = input_timestamp - rclcpp::Time(cached_pointcloud_msg.header.stamp).seconds();
