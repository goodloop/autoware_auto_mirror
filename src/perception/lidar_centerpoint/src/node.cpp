...
: Node("lidar_center_point", node_options)
{
  ...
  pointcloud_sub_ =
    this->create_subscription<sensor_msgs::msg::PointCloud2>("input/pointcloud", 1, std::bind(&LidarCenterPointNode::pointCloudCallback, this, std::placeholders::_1));
  objects_pub_ =
    this->create_publisher<autoware_perception_msgs::msg::DynamicObjectWithFeatureArray>("output/objects", 1);
  pointcloud_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("debug/pointcloud_densification", 1);
}

void LidarCenterPointNode::pointCloudCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr input_pointcloud_msg)
{
...
//   }
  ...
  for (size_t obj_i = 0; obj_i < boxes3d_vec.size() / Config::num_box_features; obj_i++) {
    ...
    if ((w * l > 2.2 * 5.5) && (w * l <= 2.5 * 7.9)) {
      feature_obj.object.semantic.type = semantic.type = autoware_perception_msgs::msg::Semantic::TRUCK;
    } else if (w * l > 2.5 * 7.9) {
      feature_obj.object.semantic.type = semantic.type = autoware_perception_msgs::msg::Semantic::BUS;
    }
