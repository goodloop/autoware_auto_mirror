...
public:
  ...
  void wait_for_subs()
  {
    while (m_pub_raw_points->get_subscription_count() < 1) {
      std::this_thread::sleep_for(std::chrono::milliseconds{1LL});
    }
  }

  bool8_t check_for_pcs(const uint32_t num_clouds)
  {
    if (m_nonground_points.size() >= num_clouds && m_ground_points.size() >= num_clouds) {
      return true;
    } else {
      return false;
    }
...
TEST(ray_ground_classifier_pcl_validation, filter_test)
{
  ...
  ray_gnd_validation_tester->wait_for_subs();
  std::this_thread::sleep_for(std::chrono::milliseconds(500LL));

  while (!ray_gnd_validation_tester->check_for_pcs(expected_num_of_pcl - 1))
  {
  ...
  while (!ray_gnd_validation_tester->check_for_pcs(expected_num_of_pcl))
  {
...
TEST(ray_ground_classifier_feature_validation, stamp_with_current_time)
{
  using Config = autoware::perception::filters::ray_ground_classifier::Config;
  using RayAggregator = autoware::perception::filters::ray_ground_classifier::RayAggregator;
  using autoware::perception::filters::ray_ground_classifier_nodes::RayGroundClassifierCloudNode;
  ...
  std::shared_ptr<RayGroundClassifierCloudNode> ray_gnd_ptr;
  std::shared_ptr<RayGroundClassifierCloudNode> ray_gnd_new_stamp_ptr;
  std::shared_ptr<RayGroundPclValidationTester> ray_gnd_validation_tester;
  ...
  const uint32_t input_cloud_size{10U};
  const uint32_t cloud_capacity{55000U};
  const char8_t * const frame_id{"base_link"};

  // ray_ground_classifier_node that passes through timestamp
  ray_gnd_ptr = std::make_shared<RayGroundClassifierCloudNode>(
    "ray_ground_classifier_cloud_node",
    raw_pcl_topic,
    ground_pcl_topic,
  ...
  if (lifecycle_msgs::msg::State::PRIMARY_STATE_INACTIVE != ray_gnd_ptr->configure().id()) {
    throw std::runtime_error("Could not configure RayGroundClassifierCloudNode 1!");
  }
  if (lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE != ray_gnd_ptr->activate().id()) {
    throw std::runtime_error("Could not activate RayGroundClassifierCloudNode 1!");
  }
  ...
  const auto pc = create_custom_pcl<float32_t>(field_names, input_cloud_size);
  ...
  while (!ray_gnd_validation_tester->check_for_pcs(1U))
  {
    ray_gnd_validation_tester->m_pub_raw_points->publish(*pc);
    // wait for ray_gnd_filter to process pc and publish data
    std::this_thread::sleep_for(std::chrono::milliseconds(500LL));
    exec.spin_some();  // for tester to collect data
  }

  // Check that both timestamps were passed through unaltered
  EXPECT_EQ(ray_gnd_validation_tester->m_nonground_points[0].header.stamp, now);
  EXPECT_EQ(ray_gnd_validation_tester->m_ground_points[0].header.stamp, now);

  // Remove previous node from spinner
  exec.remove_node(ray_gnd_ptr->get_node_base_interface());

  // Shut down the previous node
  if (lifecycle_msgs::msg::State::PRIMARY_STATE_FINALIZED != ray_gnd_ptr->shutdown().id()) {
    throw std::runtime_error("Could not shut down RayGroundClassifierCloudNode 1!");
  }

  // ray_ground_classifier_node that creates new timestamp
  ray_gnd_new_stamp_ptr = std::make_shared<RayGroundClassifierCloudNode>(
    "ray_ground_classifier_new_stamp_cloud_node",
    raw_pcl_topic,
    ground_pcl_topic,
    nonground_pcl_topic,
    frame_id,
  ...
  if (lifecycle_msgs::msg::State::PRIMARY_STATE_INACTIVE != ray_gnd_new_stamp_ptr->configure().id()) {
    throw std::runtime_error("Could not configure RayGroundClassifierCloudNode 2!");
  }
  if (lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE != ray_gnd_new_stamp_ptr->activate().id()) {
    throw std::runtime_error("Could not activate RayGroundClassifierCloudNode 2!");
  }

  // Add new node to spinner
  exec.add_node(ray_gnd_new_stamp_ptr->get_node_base_interface());

  // Update timestamp
  const auto new_now = rclcpp::Clock().now();
  pc->header.stamp = new_now;
  ...
  // Check that both timestamps were updated
  EXPECT_NE(ray_gnd_validation_tester->m_nonground_points[0].header.stamp, new_now);
  EXPECT_NE(ray_gnd_validation_tester->m_ground_points[0].header.stamp, new_now);
}
