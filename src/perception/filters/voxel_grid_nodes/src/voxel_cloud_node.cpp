  ...
  m_has_failed{false}
{
  ...
  min_point.x = static_cast<float32_t>(declare_parameter<double>("config.min_point.x"));
  min_point.y = static_cast<float32_t>(declare_parameter<double>("config.min_point.y"));
  min_point.z = static_cast<float32_t>(declare_parameter<double>("config.min_point.z"));
  voxel_grid::PointXYZ max_point;
  max_point.x = static_cast<float32_t>(declare_parameter<double>("config.max_point.x"));
  max_point.y = static_cast<float32_t>(declare_parameter<double>("config.max_point.y"));
  max_point.z = static_cast<float32_t>(declare_parameter<double>("config.max_point.z"));
  voxel_grid::PointXYZ voxel_size;
  voxel_size.x = static_cast<float32_t>(declare_parameter<double>("config.voxel_size.x"));
  voxel_size.y = static_cast<float32_t>(declare_parameter<double>("config.voxel_size.y"));
  voxel_size.z = static_cast<float32_t>(declare_parameter<double>("config.voxel_size.z"));
  const std::size_t capacity = static_cast<std::size_t>(declare_parameter<int>("config.capacity"));
