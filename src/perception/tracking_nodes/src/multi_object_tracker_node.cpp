  ...
  const common::types::bool8_t use_vision)
{
  const float32_t max_distance =
    static_cast<float32_t>(declare_parameter<double>("object_association.max_distance"));
  const float32_t max_area_ratio =
    static_cast<float32_t>(declare_parameter<double>("object_association.max_area_ratio"));
  const bool consider_edge_for_big_detections = declare_parameter<bool>(
    "object_association.consider_edge_for_big_detection");

  const auto default_variance = declare_parameter<double>("ekf_default_variance");
  const auto noise_variance = declare_parameter<double>("ekf_noise_variance");
  const std::chrono::nanoseconds pruning_time_threshold =
    std::chrono::milliseconds(
    declare_parameter<int>("pruning_time_threshold_ms"));
  const std::size_t pruning_ticks_threshold =
    static_cast<std::size_t>(declare_parameter<int>("pruning_ticks_threshold"));
  ...
  if (use_vision) {
    ...
    vision_config.intrinsics = {
      static_cast<std::size_t>(declare_parameter<double>(
    ...
    vision_config.iou_threshold = static_cast<float32_t>(declare_parameter<double>(
    ...
    vision_policy_cfg.max_vision_lidar_timestamp_diff = std::chrono::milliseconds(
      declare_parameter<int>("vision_association.timestamp_diff_ms"));
