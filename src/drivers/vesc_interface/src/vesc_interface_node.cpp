...
:  VehicleInterfaceNode{"vesc_interface", std::unordered_set<ViFeature>{}, options}
{
  ...
  set_interface(
    std::make_unique<VESCInterface>(
      *this,
      declare_parameter<double>("vesc.speed_to_erpm_gain"),
      declare_parameter<double>("vesc.speed_to_erpm_offset"),
      declare_parameter<double>("vesc.steering_angle_to_servo_gain"),
      declare_parameter<double>("vesc.steering_angle_to_servo_offset")
    )
  );
}
