  ...
  : Node(node_name, options),
    ...
    m_map_sub(
      create_subscription<MapMsgT>(
        "ndt_map",
        rclcpp::QoS{rclcpp::KeepLast{
            static_cast<size_t>(declare_parameter<int>("map_sub.history_depth")
          )}}.transient_local(),
        [this](typename MapMsgT::ConstSharedPtr msg) {map_callback(msg);})),
    m_pose_publisher(
      create_publisher<PoseWithCovarianceStamped>(
...
private:
  ...
  void init()
  {
    ...
    if (declare_parameter<bool>("load_initial_pose_from_parameters", false)) {
  ...
  geometry_msgs::msg::TransformStamped get_initial_pose()
  {
    ...
    tf.rotation.x = declare_parameter<double>("initial_pose.quaternion.x");
    tf.rotation.y = declare_parameter<double>("initial_pose.quaternion.y");
    tf.rotation.z = declare_parameter<double>("initial_pose.quaternion.z");
    tf.rotation.w = declare_parameter<double>("initial_pose.quaternion.w");
    tf.translation.x = declare_parameter<double>("initial_pose.translation.x");
    tf.translation.y = declare_parameter<double>("initial_pose.translation.y");
    tf.translation.z = declare_parameter<double>("initial_pose.translation.z");
