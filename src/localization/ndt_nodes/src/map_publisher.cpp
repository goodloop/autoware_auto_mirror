...
: Node("ndt_map_publisher_node", node_options),
  ...
  m_viz_map(declare_parameter<bool>("viz_map", false))
{
  using PointXYZ = perception::filters::voxel_grid::PointXYZ;
  PointXYZ min_point;
  min_point.x =
    static_cast<float32_t>(declare_parameter<double>("map_config.min_point.x"));
  min_point.y =
    static_cast<float32_t>(declare_parameter<double>("map_config.min_point.y"));
  min_point.z =
    static_cast<float32_t>(declare_parameter<double>("map_config.min_point.z"));
  PointXYZ max_point;
  max_point.x =
    static_cast<float32_t>(declare_parameter<double>("map_config.max_point.x"));
  max_point.y =
    static_cast<float32_t>(declare_parameter<double>("map_config.max_point.y"));
  max_point.z =
    static_cast<float32_t>(declare_parameter<double>("map_config.max_point.z"));
  PointXYZ voxel_size;
  voxel_size.x =
    static_cast<float32_t>(declare_parameter<double>("map_config.voxel_size.x"));
  voxel_size.y =
    static_cast<float32_t>(declare_parameter<double>("map_config.voxel_size.y"));
  voxel_size.z =
    static_cast<float32_t>(declare_parameter<double>("map_config.voxel_size.z"));
  const std::size_t capacity =
    static_cast<std::size_t>(declare_parameter<int>("map_config.capacity"));
