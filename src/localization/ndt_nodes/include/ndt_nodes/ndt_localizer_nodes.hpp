    ...
    ndt::P2DNDTLocalizerConfig localizer_config{
      ...
      std::chrono::milliseconds(
        static_cast<uint64_t>(
          this->template declare_parameter<int>("localizer.guess_time_tolerance_ms")))
    };

    const auto outlier_ratio{this->template declare_parameter<double>(
        "localizer.optimization.outlier_ratio")};

    common::optimization::OptimizationOptions optimizer_options{
      static_cast<uint64_t>(
        this->template declare_parameter<int>("localizer.optimizer.max_iterations")),
      this->template declare_parameter<double>("localizer.optimizer.score_tolerance"),
      this->template declare_parameter<double>("localizer.optimizer.parameter_tolerance"),
      this->template declare_parameter<double>("localizer.optimizer.gradient_tolerance")
    };

    // Construct and set the localizer.
    const float32_t step_max{static_cast<float32_t>(this->template declare_parameter<double>(
        "localizer.optimizer.line_search.step_max"))};
    const float32_t step_min{static_cast<float32_t>(this->template declare_parameter<double>(
