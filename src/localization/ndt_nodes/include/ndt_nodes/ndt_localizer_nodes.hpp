...
using OptimizerOptionsT = common::optimization::OptimizationOptions;
using PoseInitializerT = localization_common::BestEffortInitializer;
using MapT = autoware::localization::ndt::StaticNDTMap;
using OptimizationProblemT =
  autoware::localization::ndt::P2DNDTOptimizationProblem<MapT>;
using LocalizerT = localization::ndt::NdtLocalizer<OptimizerT, OptimizationProblemT>;
...
class NDT_NODES_PUBLIC P2DNDTLocalizerNode
  : public localization_nodes::RelativeLocalizerNode<
    CloudMsg, CloudMsg, LocalizerT, PoseInitializerT>
{
public:
  using LocalizerBasePtr = std::unique_ptr<localization_common::LocalizerInterface<CloudMsg, MapT>>;
      ...
      this->declare_parameter("predict_pose_threshold.rotation").template get<double>()}
  {
    ...
    const auto guess_time_tolerance{std::chrono::milliseconds(static_cast<uint64_t>(
    ...
    const auto optimizer_options{OptimizerOptionsT{
...
private:
  virtual bool on_non_convergence(
    const common::optimization::OptimizationSummary &,
  ...
  virtual bool translation_valid(
    const geometry_msgs::msg::PoseWithCovarianceStamped & pose,
